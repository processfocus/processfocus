...
const ACCESS_TOKEN = "pforg-process-list-test-token"
...
describe("pforg processes list", () => {
  ...
  test("prints only Process list nodes and maps every list flag", async () => {
    const captured: {
      requestBody: unknown
    ...
    const process = {
      id: "process-1",
      path: "/finance/purchase-request",
      name: "Purchase Request",
      category: "Finance",
      purpose: "Request a purchase",
      startStepPath: "/finance/purchase-request/Submit request",
      activeInstances: 2,
    }
    const server = Bun.serve({
      ...
      async fetch(request) {
        ...
        return Response.json({ data: { processes: { nodes: [process] } } })
      },
    })
    servers.push(server)

    const root = mkdtempSync(join(tmpdir(), "pforg-processes-"))
    tempPaths.push(root)
    ...
    expect(JSON.parse(result.stdout)).toEqual([process])
    expect(Object.keys(JSON.parse(result.stdout)[0]).toSorted()).toEqual(
      Object.keys(process).toSorted(),
    )
  ...
  test("prints an empty JSON array", async () => {
    ...
    const server = Bun.serve({
      ...
      async fetch(request) {
        captured.requestBody = await request.json()
        return Response.json({ data: { processes: { nodes: [] } } })
      },
    })
    servers.push(server)
    const root = mkdtempSync(join(tmpdir(), "pforg-empty-processes-"))
    tempPaths.push(root)
    const credentialsPath = join(root, "credentials.json")
    writeCredentials(credentialsPath)
    writeFileSync(
    ...
    const result = await runPforg(["processes", "list"], {
      PFORG_CREDENTIALS_PATH: credentialsPath,
      PF_RUNTIME_ROOT: root,
    })
  ...
  test("rejects limits above 100 before reading credentials", async () => {
    const result = await runPforg(["processes", "list", "--limit", "101"], {
