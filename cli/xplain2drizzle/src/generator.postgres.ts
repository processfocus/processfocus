...
export type GeneratorError = UnknownBaseTypeError | InvalidTypeReferenceError

/**
 * Map Xplain data type to Drizzle column definition
 */
const mapDataType = (
  columnName: string,
  dataType: DataType,
  ...
  switch (dataType.type) {
    case "text":
      columnDef = `varchar("${toSnakeCase(columnName)}", { length: ${dataType.maxLength} })`
      break
    case "boolean":
      columnDef = `boolean("${toSnakeCase(columnName)}")`
      break
    case "citext":
      columnDef = `citext("${toSnakeCase(columnName)}")`
      break
    case "datetime":
    ...
    case "integer":
      ...
      break
    case "json":
      columnDef = `jsonb("${toSnakeCase(columnName)}").$type<Record<string, unknown> | unknown[]>()`
      break
    case "real":
      columnDef = `numeric("${toSnakeCase(columnName)}", { precision: ${
        dataType.digitsBefore + dataType.digitsAfter
      }, scale: ${dataType.digitsAfter} })`
      break
    case "unlimited_text":
      columnDef = `citext("${toSnakeCase(columnName)}")`
      break
    case "urn":
      columnDef = `varchar("${toSnakeCase(columnName)}", { length: 255 })`
      break
  }
...
const generateDefaultExpression = (
  ...
  if (expr.kind === "literal") {
    if (expr.valueType === "string") {
      ...
      // SQL functions need sql template
      return { useSql: true, value: "NOW()" }
...
const generateColumn = (
  ...
  Effect.gen(function* () {
    ...
    if (referencedType) {
      ...
      let columnDef = `varchar("${columnName}", { length: ${ID_COLUMN_LENGTH} })`
      ...
      if (isSelfReference) {
        columnDef += `.references((): AnyPgColumn => ${toCamelCase(referencedType.name)}.id)`
      } else {
...
const generateIndexes = (
  ...
  for (const idx of indexes) {
    ...
    const whereClause = skipSoftDelete ? "" : ".where(sql`_deleted = false`)"
    indexDefs.push(
  ...
  for (const attr of type.attributes) {
    if (attr.isSpecialization) {
      ...
      if (referencedType) {
        ...
        const whereClause = skipSoftDelete
          ? ""
          : ".where(sql`_deleted = false`)"
        indexDefs.push(
...
const generateTable = (
  ...
  Effect.gen(function* () {
    ...
    const idColumn = type.idPrefix
      ? `  id: varchar("id", { length: ${ID_COLUMN_LENGTH} }).primaryKey().$defaultFn(() => \`${type.idPrefix}-\${ulid()}\`),`
      : `  id: varchar("id", { length: ${ID_COLUMN_LENGTH} }).primaryKey().$defaultFn(() => ulid()),`
    ...
    columns.push('  _deleted: boolean("_deleted").notNull().default(false),')
    ...
    return `export const ${constName} = pgTable("${tableName}", {\n${columns.join("\n")}\n}${indexesPart})\n`
  })
  ...
  for (const type of program.types) {
    ...
    for (const extend of type.extends) {
      ...
      if (retrieval.function === "nil" || retrieval.function === "any") {
        ...
        const viewDef = `export const ${viewConstName} = pgView("${viewName}", {
  id: varchar("id", { length: ${ID_COLUMN_LENGTH} }).notNull(),
  ${attrCamelCase}: boolean("${virtualAttrName}").notNull(),
}).as(sql\`
  select
    id,
...
`
        ...
        if (retrieval.expression) {
          ...
          if (isTypeReference) {
            // It's a foreign key
            columnDef = `varchar("${virtualAttrName}", { length: ${ID_COLUMN_LENGTH} })`
          } else {
        ...
        if (retrieval.wherePredicate) {
          ...
          subquery = generateScalarSubqueryWithWhere({
            ...
            booleanTrueValue: "true", // PostgreSQL uses native boolean
            prefix,
          })
        } else {
...
`
        ...
        // Use numeric for aggregate results (PostgreSQL)
        const columnDef = "numeric"
...
export const generateTriggerSQL = (
  ...
  lines.push("-- Reusable function for updated_at")
  lines.push("CREATE OR REPLACE FUNCTION update_updated_at_column()")
  lines.push("RETURNS TRIGGER AS $$")
  lines.push("BEGIN")
  lines.push("  IF NEW.updated_at = OLD.updated_at THEN")
  ...
  lines.push("-- Triggers for all tables")
...
/**
 * Generate the complete Drizzle schema file
 */
export const generateDrizzleSchema = (
  ...
  Effect.gen(function* () {
    ...
    const imports = new Set<string>([
      "pgTable",
      "varchar", // Always needed for ID columns
    ])

    if (needsViews) {
      imports.add("pgView")
      imports.add("boolean") // nil/any functions return boolean
      imports.add("integer") // count function returns integer
    }
    if (needsNumeric) {
      imports.add("numeric") // total/min/max functions return numeric
    }
    ...
    // Track if we need citext custom type
    let needsCitext = false
    ...
    // Check if we have any self-references (need AnyPgColumn type)
    ...
    if (typeImports.length > 0) {
      lines.push(
        `import type { ${typeImports.join(", ")} } from "drizzle-orm/pg-core"`,
      )
    }
    lines.push(
      `import { ${valueImports.join(", ")} } from "drizzle-orm/pg-core"`,
    )
    if (hasDateTimeColumns) {
    ...
    if (hasDateTimeColumns) {
      ...
      lines.push("  dataType() {")
      lines.push('    return "timestamp without time zone"')
      lines.push("  },")
      lines.push("  toDriver(value: DateTime.Utc): Date {")
      lines.push("    return DateTime.toDateUtc(value)")
      lines.push("  },")
      lines.push("  fromDriver(value: Date | string): DateTime.Utc {")
      lines.push("    return parsePgTimestamp(value)")
      lines.push("  },")
      lines.push("})")
      lines.push("")
    }
