...
import { SqliteProcessExecutionOperationsLive } from "../src/lib/process-execution"
...
describe("process-execution", () => {
  ...
  it("persists a caller-supplied executionId", () =>
    runTest(
      Effect.gen(function* () {
        const db = yield* TypedSqliteDrizzle
        ...
        const executionId = `pex-sqlite-${suffix}`
  ...
  it("rolls back the extra process state row on duplicate executionId insert", () =>
    runTest(
      Effect.gen(function* () {
        const db = yield* TypedSqliteDrizzle
        ...
        const executionId = `pex-sqlite-dup-${suffix}`
