...
describe("authored invitation hydration", () => {
  it("adds pending model invitations with roles and source=model", () =>
    runTest(
      Effect.gen(function* () {
  ...
  it("soft-deletes removed pending model invitations and their roles in one hydrate", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
        ...
        const staleRoles = yield* Effect.gen(function* () {
          const db = yield* TypedSqliteDrizzle
          return yield* db
            .select()
            .from(schema.invitationRole)
  ...
  it("updates pending model invitation email and roles on edit", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
        ...
        const allRoles = yield* Effect.gen(function* () {
          const db = yield* TypedSqliteDrizzle
          return yield* db
            .select({
  ...
  it("does not rewrite, delete, or reopen accepted model invitations on remove or change", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
        ...
        // Model still lists the construct with a different email/role — must not
        ...
        // Model omits the construct entirely — accepted history still remains.
  ...
  it("does not soft-delete legacy-closed model invitations when omitted from the model", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
  ...
  it("requires a new construct identity for another grant after acceptance", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
  ...
  it("leaves dashboard, process, and legacy invitations alone during model orphan cleanup", () =>
    runTest(
      Effect.gen(function* () {
        // Hydrate a model invitation so a stable role exists for non-model seeds.
        yield* storeOrganisation(
          makeOrgWithInvitations([
            {
              id: "model-only",
        ...
        // Remove only the model-owned invitation; keep the role via a retained model invite path
        // that reuses the same role construct id by re-importing an empty invitation list while
        // the role remains through a companion retained model invitation below.
        yield* storeOrganisation(
          makeOrgWithInvitations([
            {
              id: "retained-model",
  ...
  it("remove/re-add reactivates only currently declared roles for a model invite", () =>
    runTest(
      Effect.gen(function* () {
        yield* storeOrganisation(
          makeOrgWithInvitations([
