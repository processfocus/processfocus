...
import { Data, DateTime, Effect, Either, Layer, Option, Stream } from "effect"
import {
...
/** SHA-256 of base64url token bytes, encoded base64url — same shape as Registration Link. */
...
import { SqliteDelegationDatabaseLive } from "./delegation-database.js"
import {
  isTransientSqliteError,
  retryTransientSqliteError,
} from "./transient-sqlite-error"

export { isSqliteBusy, isTransientSqliteError } from "./transient-sqlite-error"
...
export const removeExpiredOAuthStorage = Effect.gen(function* () {
  ...
  const deleted = yield* db
    ...
    .where(
      and(
        not(isNull(schema.oauthStorage.keyExpiry)),
        lte(schema.oauthStorage.keyExpiry, now),
      ),
    )
    .returning({ id: schema.oauthStorage.id })
...
const AuthenticationDatabaseLive = Layer.effect(
  AuthenticationDatabase,
  Effect.gen(function* () {
    const db = yield* TypedSqliteDrizzle
    ...
    return {
          ...
          const removed = yield* db
            ...
            .where(
              and(
                ...
                eq(schema.passkeyCredential.userId, input.userId),
                eq(schema.passkeyCredential._deleted, false),
                // Keep the count inside the UPDATE so concurrent writers cannot
                // both remove the final pair of credentials.
                gt(
      ...
      exchangeRegistrationLink: (rawToken: string) =>
        Effect.gen(function* () {
          ...
          // Passkey provider must be enabled.
          ...
          // Link expiry is enforced at exchange only.
          if (DateTime.lessThanOrEqualTo(invitation.expiresAt, now)) {
            return Option.none<RegistrationSessionExchangeResult>()
          }

          // Require at least one active role assignment.
          ...
          // Normalized email must be unowned.
      ...
      upsertM2MUser: (
        ...
        Effect.gen(function* () {
              ...
              .pipe(
                ...
                Effect.catchAll((error) =>
                  isTransientSqliteError(error)
                    ? Effect.logWarning(
                        "Non-critical last_logged_in update failed, continuing authentication",
                      ).pipe(
                        Effect.annotateLogs({ error, clientId }),
                        Effect.as([] as Array<{ id: string }>),
                      )
                    : Effect.fail(error),
                ),
              )
          ...
          const existing = yield* findExistingM2MUser()

          if (existing[0]) {
            return existing[0].id
          }

          // Row doesn't exist: insert and handle races safely.
          const insertM2MUser = db.insert(schema.user).values({
          ...
          const insertResult = shouldUpdateLastLoggedIn
            ? yield* insertM2MUser
                .onConflictDoUpdate({
                  target: [schema.user.provider, schema.user.sub],
                ...
                .returning({ id: schema.user.id })
                .pipe(retryTransientSqliteError, Effect.either)
            : yield* insertM2MUser
                .returning({ id: schema.user.id })
                .pipe(retryTransientSqliteError, Effect.either)

          if (Either.isLeft(insertResult)) {
            const racedExisting = yield* findExistingM2MUser()

            if (racedExisting[0]) {
              return racedExisting[0].id
            }

            return yield* Effect.fail(insertResult.left)
          }

          const newUser = yield* returnedRow(insertResult.right)
          return newUser.id
...
export const SqliteAuthenticationDatabaseLive = Layer.merge(
  AuthenticationDatabaseLive,
  SqliteDelegationDatabaseLive,
)

export const SqliteOpenAuthStorageServiceLive = Layer.effect(
  StorageService,
  Effect.gen(function* () {
    const db = yield* TypedSqliteDrizzle
