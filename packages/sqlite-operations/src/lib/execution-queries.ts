...
/**
 * Live implementation of ExecutionQueries service for SQLite.
 * Provides pull operations for RxDB replication.
 */
export const SqliteExecutionQueriesLive = Layer.effect(
  ExecutionQueries,
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        const rows = yield* db
          .select({
            ...
            updatedBy: schema.toDo.updatedBy,
    ...
    const queryExecutions = (options: {
      ...
      Effect.gen(function* () {
        ...
        // Julian day to ISO 8601: use strftime to format with T separator and Z suffix
        const baseQuery = db
          .with(durationStats)
          .select({
            ...
            finishedAt: sql<
              string | null
            >`CASE WHEN ${schema.processExecution.finishedAt} IS NULL THEN NULL ELSE strftime('%Y-%m-%dT%H:%M:%SZ', ${schema.processExecution.finishedAt}) END`.as(
              "finished_at_iso",
            ),
            abandonedAt: sql<
              string | null
            >`CASE WHEN ${schema.processExecution.abandonedAt} IS NULL THEN NULL ELSE strftime('%Y-%m-%dT%H:%M:%SZ', ${schema.processExecution.abandonedAt}) END`.as(
              "abandoned_at_iso",
            ),
            ...
            updatedAt:
              sql<number>`CAST(ROUND((${schema.processExecution.updatedAt} - 2440587.5) * 86400000) AS INTEGER)`.as(
                "updated_at_millis",
              ),
            ...
            finishedAtMs: sql<
              number | null
            >`CASE WHEN ${schema.processExecution.finishedAt} IS NULL THEN NULL ELSE (${schema.processExecution.finishedAt} - 2440587.5) * 86400000 END`.as(
              "finished_at_millis",
            ),
            // Start step info
            ...
            processStateCreatedAt:
              sql<string>`strftime('%Y-%m-%dT%H:%M:%SZ', ${schema.processState.createdAt})`.as(
    ...
    const buildCheckpointFilter = (
      ...
      const updatedAtMillis = sql<number>`CAST(ROUND((${schema.processExecution.updatedAt} - 2440587.5) * 86400000) AS INTEGER)`
