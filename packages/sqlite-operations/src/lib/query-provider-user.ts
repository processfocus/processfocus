...
import { permittedRole } from "@pf/drizzle-sqlite"
import {
...
/**
 * SQLite implementation of ProviderUserQueries service
 */
export const SqliteProviderUserQueriesLive = Layer.effect(
  ProviderUserQueries,
  Effect.gen(function* () {
              ...
              trimmed.length === 0
                ...
                : {
                    ...
                    OR: [
                      {
                        RAW: (providerUser) =>
                          sql`${providerUser.name} like ${`%${escapeLikePattern(trimmed)}%`} escape ${"\\"}`,
                      },
                      {
                        RAW: (providerUser) =>
                          sql`${providerUser.email} like ${`%${escapeLikePattern(trimmed)}%`} escape ${"\\"}`,
                      },
                    ],
                  },
