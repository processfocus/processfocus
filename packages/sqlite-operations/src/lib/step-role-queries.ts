...
/**
 * Live implementation of StepRoleQueries service for SQLite.
 * Now queries the step's role directly from the step table (no junction table).
 */
export const SqliteStepRoleQueriesLive = Layer.effect(
  StepRoleQueries,
  Effect.gen(function* () {
    const db = yield* TypedSqliteDrizzle

    // Note: querySupportingRolePaths is intentionally duplicated with
    // postgres-operations/src/lib/step-role-queries.ts. Keep both in sync.
