...
it.layer(TestLayer, { timeout: "60 seconds" })(
  "org import regression",
  (it) => {
    it.effect(
      "preserves delimiter-bearing relationships and removes genuine orphans",
      () =>
        Effect.gen(function* () {
          ...
          const nestedUnit = orgUnits.find(
            (unit) => unit.path === "/records->unit",
          )
          ...
          expect(responsibilities.filter((row) => !row._deleted)).toHaveLength(
            1,
          )
          ...
          expect(
            holidays.find((row) => row.holidayTitle === "Founders: Day")
              ?._deleted,
          ).toBe(false)
          expect(
            holidays.find((row) => row.holidayTitle === "Old: Holiday")
              ?._deleted,
          ).toBe(true)
          ...
          const supportingRoles = yield* db
            .select()
