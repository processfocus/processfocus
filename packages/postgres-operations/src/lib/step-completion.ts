...
const epochMillisToDateTime = (epochMillis: number | string) => {
  const millis = Number(epochMillis)
  if (!Number.isFinite(millis)) {
    throw new TypeError(`Expected epoch milliseconds, got ${epochMillis}`)
  }
  return DateTime.unsafeFromDate(new Date(millis))
}

/**
 * Live implementation of StepCompletionOperations service for PostgreSQL.
 *
 * Contains only database operations without business logic.
 */
export const PostgresStepCompletionOperationsLive = Layer.effect(
  StepCompletionOperations,
  Effect.gen(function* () {
    ...
    return {
      ...
      updateProcessState: (
        ...
        Effect.gen(function* () {
          ...
          // PostgreSQL: Perform deep merge in application code
          // The || operator only does shallow merge at top level, which would lose nested data.
          // Example: {user: {name: "John", age: 30}} || {user: {age: 31}} = {user: {age: 31}} (loses name!)
          // Deep merge ensures nested objects are preserved: {user: {name: "John", age: 31}}
          //
          // Use FOR UPDATE to lock the row during read-modify-write cycle.
          ...
          const existingState =
            (currentState[0]?.state as Record<string, unknown>) ?? {}
          const mergedState = deepMerge(existingState, data)
      ...
      updateProcessStateIfUnchanged: (
        ...
        Effect.gen(function* () {
          ...
          const existingState = Array.isArray(current.state)
            ? {}
            : current.state
          const rows = yield* db
            .update(schema.processState)
            .set({
              state: deepMerge(existingState, data),
              updatedAt,
              updatedBy: userDetails.by,
            })
            .where(
            ...
            .returning({ updatedAt: schema.processState.updatedAt })

          const row = rows[0]
          return row
            ? { kind: "updated" as const, updatedAt: row.updatedAt }
      ...
      appendProcessStateArrayItem: (
        ...
        Effect.gen(function* () {
          ...
          yield* db
            .update(schema.processState)
            .set({
              state: sql`jsonb_set(COALESCE(${schema.processState.state}, '{}'::jsonb), ARRAY[${stateKey}], COALESCE(${schema.processState.state} -> ${stateKey}, '[]'::jsonb) || jsonb_build_array(${itemJson}::jsonb), true)`,
              updatedAt: requestTime,
              updatedBy: userDetails.by,
            })
            .where(eq(schema.processState.id, processStateId))
        ...
        Effect.gen(function* () {
          ...
          const rows = yield* db
            .select({
              ...
              // Pre-calculated SLA target and warning times
              slaTargetAt: sql<
                string | null
              >`CASE WHEN ${schema.toDo.slaTargetAt} IS NOT NULL THEN TO_CHAR(${schema.toDo.slaTargetAt} AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') ELSE NULL END`.as(
                "sla_target_at_iso",
              ),
              slaWarningAt: sql<
                string | null
              >`CASE WHEN ${schema.toDo.slaWarningAt} IS NOT NULL THEN TO_CHAR(${schema.toDo.slaWarningAt} AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.FF3"Z"') ELSE NULL END`.as(
                "sla_warning_at_iso",
              ),
              deleted: schema.toDo._deleted,
            })
        ...
        Effect.gen(function* () {
          const result = yield* db
            .select({
              ...
              // PostgreSQL: Extract epoch and convert to milliseconds
              processStartedAt:
          ...
          return {
            ...
            processStartedAt: epochMillisToDateTime(result[0].processStartedAt),
          }
        }),
          ...
          const result = yield* db
            ...
            .where(
              and(
                ...
                sql`${schema.jobQueue.jobPayload}->>'processExecutionId' = ${processExecutionId}`,
              ),
            )
          return Number(result[0]?.count ?? 0)
        }),
        ...
        Effect.gen(function* () {
          ...
          for (const result of results) {
            ...
            validated.push({
              ...
              processStartedAt: epochMillisToDateTime(result.processStartedAt),
        ...
        Effect.gen(function* () {
          ...
          return allResults.map(
            (row): CompletedStepData => ({
              ...
              completedAt: epochMillisToDateTime(row.completedAt),
            }),
          )
        }),
        ...
        Effect.gen(function* () {
          const results = yield* db
            .select({
              ...
              stepPath: schema.step.path,
              isSystemStep:
                sql<boolean>`CASE WHEN ${schema.step.roleId} IS NULL THEN TRUE ELSE FALSE END`.as(
                  "is_system_step",
                ),
            })
            .from(schema.toDo)
