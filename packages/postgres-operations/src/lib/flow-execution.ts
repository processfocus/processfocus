...
/**
 * Live implementation of FlowExecutionOperations service for PostgreSQL.
 *
 * Contains only database operations without business logic.
 */
export const PostgresFlowExecutionOperationsLive = Layer.effect(
  FlowExecutionOperations,
  Effect.gen(function* () {
    ...
    return {
          ...
          const result = yield* db
            ...
            .where(
              and(
                ...
                sql`(
                  (
                    ${schema.jobQueue.jobPayload}->>'sourceScheduledFlowId' IS NULL
                    AND ${schema.jobQueue.jobPayload}->>'processExecutionId' = ${processExecutionId}
                  )
                  OR
                  (
                    ${schema.jobQueue.jobPayload}->>'sourceScheduledFlowId' IS NOT NULL
                    AND ${schema.jobQueue.jobPayload}->>'processExecutionId' = ${processExecutionId}
                    AND EXISTS (
                      SELECT 1 FROM ${schema.scheduledFlow}
                      WHERE ${schema.scheduledFlow.id} = ${schema.jobQueue.jobPayload}->'payload'->>'scheduledFlowId'
        ...
        Effect.gen(function* () {
          const rows = yield* db
            ...
            .where(
              and(
                eq(schema.jobQueue._deleted, false),
                sql`${schema.jobQueue.jobPayload}->>'sourceScheduledFlowId' = ${sourceScheduledFlowId}`,
              ),
            )
          return rows
            .map(({ id, payload }) => ({
      ...
      queryFlowDispatchIdsByStorageQueue: (storageQueue, now, limit) =>
        Effect.gen(function* () {
          const sourceScheduledFlowId = sql<string>`${schema.jobQueue.jobPayload}->>'sourceScheduledFlowId'`
          const rows = yield* db
      ...
      getExecutionDetailsForCompletion: (processExecutionId: string) =>
        Effect.gen(function* () {
          const result = yield* db
            .select({
              // PostgreSQL: Extract epoch in milliseconds
              createdAtMs:
