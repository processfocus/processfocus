...
const isActiveProviderUserRoleConflict = (error: unknown): boolean => {
  ...
  // Match the current unique index text so concurrent grants do not surface
  // a SQL error from the CLI.
  return (
...
/**
 * PostgreSQL implementation of SettingsQueries service
 */
export const PostgresSettingsQueriesLive = Layer.effect(
  SettingsQueries,
  Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          yield* db
            ...
            .onConflictDoUpdate({
              ...
              targetWhere: eq(schema.userSettings._deleted, false),
              set: {
                notificationPreference:
                  serializeNotificationPreferences(preferences),
                updatedAt: now,
