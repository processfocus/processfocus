...
import { fetchProviderJson } from "./provider-api.js"
...
export function SlackProvider(config: SlackConfig) {
  return Oauth2Provider({
    ...
    mapVerifiedHumanIdentity: (payload) =>
      verifiedOidcIdentity("slack", payload),
    resolveHumanIdentity: async (tokenset) => {
      const claims = await fetchProviderJson(
        "https://slack.com/api/openid.connect.userInfo",
        tokenset,
      )
      const idTokenSubject = tokenset.id?.["sub"]
      if (
        typeof idTokenSubject !== "string" ||
        claims["sub"] !== idTokenSubject
      ) {
        throw new Error("Slack UserInfo subject does not match ID token")
      }
      const verifiedIdentity = verifiedOidcIdentity("slack", claims)
      return {
        claims,
